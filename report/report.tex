	  ...
	  In this game, every player have a character, called a hero, who has characteristics, spells, and weapons.\\
		...
		%  Our supervisor, Richard Foley, offered us a project's subject based on web and mobile
		 ...
		 % during our studies.\\
		  ...
		  The project that we choose is a 2D Multi-player video game.\\
		...
		%  review this project and the skills acquired during these three months.
		 ...
		 lot of thing to do in Glasgow. There are a lot of museums, parks, and monuments to
		 visit.\\
...
We implemented several layouts for all of our needs.
	...
	The class style change with the component, for example : Buttons and textfields share the same style: graphics::BasicStyle. This style represents a rectangle : it takes 2 arguments, the path to the border's texture and the path to the center's texture.\\
	...
	Each column is a posture, and each row is a direction. With two int in as attribute, we can manage every posture in every direction with a simple addition. For example to have the first step to the north we have :\\
		...
		In order to display all components, we use a buffer called \emph{Buffer Y}. At each frame, this buffer is emptied, and is filled with refreshed components (with their new positions, sizes and states). A component must inherit from \emph{graphics::BufferDrawable} to be put in the Buffer Y. This class has two methods:
			...
			Most of animations we implemented are used for spells : the class graphics::SingleTargetSpellAnim, inherite from Animation, take as argument the caster(character which cast the spell) and the target of the spell. This class have a static method process which allow to instanciate the right class depend on a spell's id.\\
			Spells' animation are implemented in graphics::SingleTargetSpellImpl. For projectiles, we create graphics::ShotAnim which inherite from graphics::SingleTargetSpellAnim. It able to determinate the projectile's position with the angle between the caster and the target, and its speed.\\
			There is also LifeLose which display looses life point.\\
			...
			RECHECK PLEASE \\
				...
				\item[graphics::Player:]{Inherit from graphics::Character, which calculate character's position depending on its speed, previous coordinates, direction, and elapsed time since last frame}
				\item[graphics::UserPlayer:]{Inherit from graphics::Player, represents client's character. The event's managing is different and add some graphics elements. (range in the green circle for example.)}
				\end{description}

				About character's movements, we decided to let the client calculate the path to move from its position to the place where the player clicked, to avoid a server overload. However, clients' positions are always checked by the server to make sure they are not cheating. When the client want to move, it send GAME\_ASKMOVE) with the number of coordinate and the ist of coordinates generated by the pathfinding. Then, for each clients' update, the server calculate the current position and his direction. If client's direction has changed, the server send to the client list a packet GAME\_PLAYERMOVE with the client's id, his coordinates and his direction.\\
			The exchange on the network is not instantaneous and different for each client, we had to add a offset on the coordinate calculated by the server in order to make more fluid the movement.\\

			About character's target, the client send a packet GAME\_ASKTARGET with the target's id, or 0 if it haven't any target. If the target is okay,  the server answer with a packet  GAME\_ANSWERTARGET to confirm.
			...
			Since both the server and the client have network functionnalities in common, instead of writing these function twice, we decided to write a shared library. This library contains the network protocol.\\
				...
				We implementes 3 data types available to be send :
				...
				If client is successfuly logged in, it goes to character-selection screen and send a \emph{SESSION\_ASKCHARACTER} message. Afterwards, the server sends a \emph{SESSION\_ANSWERCHARACTER} for each character the client owns, containing character's id, name, level and experience.\\
					Each Client object contains a \emph{Player} object as attribute. This attribute represent the character choosed by client, while the client doesn't select a character, its value is NULL. To update this value, the server must receive a \emph{SESSION\_SELECTCHARACTER} followed by the character id from the client who wants to select a character. The server make sure that the character exists and is associate to the client before changing the Player attribute's value and load all character's information from database.

					\subsection{Skills}
					When a client want to see the tree, it sends a SESSION\_ASKALLSKILLS message. Then, the server reply with a SESSION\_ANSWERALLSKILLS containing the number of skills the client bought and their IDs. The clients update its skill-tree afterwards, by circling in blue bought skills, in yellow for those it's allowed to buy, and in grey for those which it can't buy yet.
					When a client wants to buy a new skill, it sends a SESSION\_ASKSKILL packet containing skill ID, if purchase is accepted, the server reply with a SESSION\_ANSWERSKILL, which also contain skill ID.\\

					For the server, each Player object contains a skill-ID-vector which represent all skills the player owns. To get the informations, the server send the skill's id. In the class SkillImpl, with the method process, which update player's statistics and spells. For that, SkillImpl got a map which associate the id to a function  containing spell's effects.
