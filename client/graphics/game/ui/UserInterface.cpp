...
UserInterface::UserInterface(game::Player* player) : m_boolMenu(false) , m_gotTarget(false), m_loaded(false){
	m_player = player;
	m_frame_mode = NULL;
...
void UserInterface::validate() {
	...
	//m_chat->setSize(300,200);
	m_chat->setCoord(util::CoordInt(getParent()->getWidth()-300,getParent()->getHeight()-200));
}
