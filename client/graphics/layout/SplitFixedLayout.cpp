	...
	void SplitFixedLayout::validate() {
		...
		if(m_container->childSize() == 2) {
			if(m_direction == horizontal) {
				for(int i=0; i<2; i++) {
					if(i == m_fixed) {
						m_container->getChild(i)->setCoord(util::Coordinates(m_fixedSize, m_container->getHeight()));
					}
					else {
						m_container->getChild(i)->setCoord(util::Coordinates(m_container->getWidth()-m_fixedSize, m_container->getHeight()));
					}
				}
			}
			else {
